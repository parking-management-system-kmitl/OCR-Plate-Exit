...
def process_frame(frame_queue, model, last_ocr_time, trigger_zone, model_path, font_path, ocr_results_queue):
    ...
    while not stop_event.is_set():
        try:
                                    ...
                                    if current_time - last_ocr_time >= 1:  # 1 second delay between OCR attempts
                                        ...
                                        if plate_img.size > 0:  # Check if plate image is valid
                                            try:
                                                ...
                                                # แสดงสถิติทุกครั้งที่ทำ OCR เสร็จ
                                                min_ocr_time = min(ocr_times)
                                                max_ocr_time = max(ocr_times)
                                                avg_ocr_time = sum(ocr_times) / len(ocr_times)
                                                
                                                print("\n" + "="*50)
                                                print(f"OCR Statistics (n={ocr_count}):")
                                                print(f"Current OCR time: {ocr_time*1000:.2f} ms")
                                                print(f"Minimum OCR time: {min_ocr_time*1000:.2f} ms")
                                                print(f"Average OCR time: {avg_ocr_time*1000:.2f} ms")
                                                print(f"Maximum OCR time: {max_ocr_time*1000:.2f} ms")
                                                print("="*50)
            ...
            # แสดงเวลา OCR เฉลี่ยบนภาพ
            if ocr_times:
                avg_ocr_time = sum(ocr_times) / len(ocr_times)
                cv2.putText(display_frame, f"Avg OCR: {avg_ocr_time*1000:.2f} ms (n={ocr_count})", 
            ...
            if key == ord('q'):
                ...
                # แสดงสรุปเวลา OCR ก่อนปิดโปรแกรม
                if ocr_times:
                    avg_ocr_time = sum(ocr_times) / len(ocr_times)
                    min_ocr_time = min(ocr_times)
                    max_ocr_time = max(ocr_times)
                    print("\n" + "="*50)
                    print(f"OCR Statistics Summary:")
